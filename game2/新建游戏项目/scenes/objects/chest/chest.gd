...
@onready var animated_sprited_2d: AnimatedSprite2D = $AnimatedSprited2D
...
func on_interactable_deactivated() -> void:
	if is_chest_open:
		animated_sprited_2d.play("chest_close")
...
func _unhandled_input(event: InputEvent) -> void:
	if in_range and event.is_action_pressed("show_dialogue"):
		interactable_label_component.hide()
		animated_sprited_2d.play("chest_open")
		is_chest_open = true
...
func add_reward_scene() -> void:
	for scene in output_reward_scenes:
		var reward_scene: Node2D = scene.instantiate() as Node2D
...
func get_random_position_in_circle(center: Vector2, radius: int) -> Vector2i:
	var angle = randf() * TAU
	var distance_from_cneter = sqrt(randf()) * radius
	var x: int = center.x + distance_from_cneter * cos(angle)
	var y: int = center.y + distance_from_cneter * cos(angle)
